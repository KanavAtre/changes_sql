...
con =mycon.connect(host="localhost",user="root",passwd="kanav@04") 


if con.is_connected():
    print("Connected to mysql succesfully")
    cur = con.cursor()
    cur.execute("create database game_data_6")
    cur.execute("use game_data_6")
    cur.execute("create table scoreboard(UserName varchar(50), Score integer(5))")
...
icon = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/games.png')
pygame.display.set_icon(icon)

mixer.music.load("/Users/thispc/Downloads/Space-Invader-main/background.wav")
mixer.music.play(-1)
...
def user_data():
    ...
    L1=Label(game_win, text="Enter user name: ")
    L1.place(x=CENTER-30, y=CENTER)

    E1=Entry(game_win)
    E1.place(x=CENTER, y=CENTER)
    data_name=E1.get()
...
def game():
    ...
    # Player
    playerImg = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/spaceship.png')
    ...
    # Enemy
    enemyImg = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/alien.png')
    ...
    # Background
    bgImg = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/background.jpg')
    bgx = 0
    bgy = 0

    # Bullet
    bulletImg = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/ammunition.png')
    ...
    # Enemy bullet
    ebulletImg = pygame.image.load('/Users/thispc/Downloads/Space-Invader-main/bullet.png')
...
class Button():
    ...
    def check(self, position):
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
            ...
            cur.execute("insert into values scoreboard(value_name)")
            game()
   
    def quit(self, position):
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
            cur = con.cursor()
            cur.execute("drop database game_data_6")
            pygame.quit()
            quit()
    ...
    def scores(self, position):
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
            value_out=cur.execute("select * from scoreboard")
            print(value_out)


button_surface = pygame.image.load("/Users/thispc/Downloads/Space-main-2/button2.png")
...
while True:
    ...
    bg_img=pygame.image.load("/Users/thispc/Downloads/Space-Invader-main/background.jpg")
